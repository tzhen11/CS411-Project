...
if(!empty($username)){
    if(!empty($password)){
        ...
        if(mysqli_connect_error()){
            ...
            $sql="SELECT username FROM testing_db WHERE username='$username'";
            $result = $conn->query($sql);
            if($result->num_rows > 0){
                echo "This username already exists, please return and enter a different username!";
                die();
            }
            else{
                ...
                if($conn->query($sql)){
                    echo "Signup successful, please return to main site to log in!";
                }
                else{
